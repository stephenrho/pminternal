...
ridge_fun <- function(data, ...){
  ...
  cv <- cv.glmnet(x=x, y=y, alpha=0, nfolds = 10, family="binomial")
  lambda <- cv$lambda.min
  
  glmnet(x=x, y=y, alpha = 0, lambda = lambda, family="binomial")
}
...
lognet_fun <- function(data, ...){
  ...
  cv <- cv.glmnet(x=x, y=y, alpha = alpha, nfolds = 10, family="binomial")
  lambda <- cv$lambda.min
  
  glmnet(x=x, y=y, alpha = alpha, lambda = lambda, family="binomial")
}
...
enet_fun <- function(data, ...){
  ...
  res <- lapply(alphas, function(a){
    cv <- cv.glmnet(x=x, y=y, alpha = a, nfolds = 10, family="binomial")
  ...
  glmnet(x=x, y=y, alpha = alphas[j], lambda = res[[j]][["lambda"]], family="binomial")
}
...
rf_fun <- function(data, ...){
  ...
  ranger(y~., data = data, probability = T, 
         num.trees = num.trees, 
         max.depth = max.depth, 
